...
def RandomSearch(num_items, items, capacity, num_conflicts, conflicts):
    ...
    for deep in range(200000):    
        ...
        # Se a máscara está com metade do peso agendamos inserções de items para análise
        if memory > RAM and (capacity/(capacity_calc + 0.1)) <= 2:    
        ...
        if conflict_true == False: 
            if value > best_solution_value and capacity_calc >= 0:
                ...
                #mask_agenda.clear()
                mask_agenda += Add(mask,items_weight,capacity_calc,1)
                mask_agenda += Remove(mask,1)
                if DEBUG >= 1:
